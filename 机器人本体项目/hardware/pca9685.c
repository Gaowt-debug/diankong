...
#define PCA_SCL_PORT    GPIOB
#define PCA_SCL_PIN     GPIO_Pin_9
#define PCA_SDA_PORT    GPIOB
#define PCA_SDA_PIN     GPIO_Pin_8
...
void PCA9685_SetDuty(uint8_t channel, uint16_t duty) {
	if(duty>=496&&duty<=512 )
		
	{
		duty-=duty%2 ;
	}
	if(duty>=513&&duty<=522 )
		
	{
		duty-=duty%2 ;
	}
	if(duty>=220&&duty<=270 )
